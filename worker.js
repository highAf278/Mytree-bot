...
function cupcakeButtons(used=[]){const usedSet=new Set(Array.isArray(used)?used:[]);const available=BIRTHDAY_CUPCAKE_INGREDIENTS.filter(x=>!usedSet.has(x[0]));const missingRecipe=BIRTHDAY_SECRET_RECIPES.flatMap(r=>r.need).filter(n=>!Array.from(usedSet).some(u=>String(u).includes(n)));const guaranteed=available.filter(x=>missingRecipe.some(n=>String(x[0]).includes(n)));const pool=available.filter(x=>!guaranteed.includes(x));const shuffledPool=[...pool].sort(()=>Math.random()-.5);const shuffled=[...guaranteed.sort(()=>Math.random()-.5).slice(0,Math.min(guaranteed.length,10)),...shuffledPool].slice(0,10).sort(()=>Math.random()-.5);const rows=[row(...shuffled.slice(0,5).map(x=>button(x[0],`birthday:cupcakepick:${encodeURIComponent(x[0])}`,1))),row(...shuffled.slice(5,10).map(x=>button(x[0],`birthday:cupcakepick:${encodeURIComponent(x[0])}`,1)))].filter(r=>r.components.length);rows.push(row(button("⏭️ Skip Layer","birthday:cupcakeskip",2)));return rows;}
