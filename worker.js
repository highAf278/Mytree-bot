...
function cupcakeButtons(used=[]){const usedSet=new Set(Array.isArray(used)?used:[]);const available=BIRTHDAY_CUPCAKE_INGREDIENTS.filter(x=>!usedSet.has(x[0]));const shuffled=[...available].sort(()=>Math.random()-.5).slice(0,10);return [row(...shuffled.slice(0,5).map(x=>button(x[0],`birthday:cupcakepick:${encodeURIComponent(x[0])}`,1))),row(...shuffled.slice(5,10).map(x=>button(x[0],`birthday:cupcakepick:${encodeURIComponent(x[0])}`,1)))].filter(r=>r.components.length);}
